...
#I was trying to select accession numer and genes with significantly different gene length
...
p_value = 0.05
def make_distribution(length_list):
	tail_values = []
	for i in [p_value,(1-p_value)]:
		tail_values.append(round(norm.ppf(i, loc=np.mean(length_list), scale=np.std(length_list))))
	return tail_values
...
for gene in mito_genes:
	...
	for file in gene_files:
		record = SeqIO.read(file,"fasta")
		count = 0
		if len(record.seq) < distribution_tails[0] or len(record.seq) > distribution_tails[1]:
			#print(record.id + ' unusual length')
			tab_dict[record.id].append('Y')
			count += 1
		tab_dict[record.id].append('-')
	#print(count)
	#print('--------------------------')
